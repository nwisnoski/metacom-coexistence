...
#   arrange(patch)
# write_csv(last_t_out, path = paste0("out/last_t_above_stabilizing_", tstamp,".csv"))
# 
# last_t_out <- dynamics_total %>%
#   filter(time == max(dynamics_total$time),
#          D > 0) %>%
#   group_by(dispersal, germination, survival, patch) %>%
#   arrange(patch)
# write_csv(last_t_out, path = paste0("out/last_t_below_stabilizing_", tstamp,".csv"))
# analyze diversity
# last_t_out <- dynamics_total %>% 
#   filter(time == max(dynamics_total$time),
#          N > 0) %>% 
...
#   ggsave("figures/aboveground_diversity_partitioning_stabilizing2000_ext005.pdf", width = 8, height = 6)
# 
# 
# last_t_out <- dynamics_total %>% 
#   filter(time == max(dynamics_total$time),
...
#   scale_x_log10() +
#   theme(legend.position = "top") +
#   ggsave("figures/belowground_diversity_partitioning_stabilizing2000_ext005.pdf", width = 8, height = 6)
# 
# 
# stopCluster(cl)
